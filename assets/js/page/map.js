...
function setMarkers(map, items) {
  ...
  const pins = {};
  pins['default'] = { ...icon, url: icon.url + 'default.png' };

  const markers = [];

  items.forEach((item) => {
    let currentIcon = pins['default'];
    if (item.loc.category && !pins[item.loc.category.icon]) {
      pins[item.loc.category.icon] = { ...icon, url: icon.url + item.loc.category.icon + '.png' };
    }
    if (item.loc.category) {
      currentIcon = pins[item.loc.category.icon];
    }

    const marker = new google.maps.Marker({
      position: { lat: Number.parseFloat(item.loc.lat), lng: Number.parseFloat(item.loc.lon) },
      icon: currentIcon,
      shape: shape,
      title: item.loc.name,
      type: item.loc.category,
      zIndex: currentIcon.zIndex,
    });
...
function searchCoord(map, inputWrapper) {
  ...
  if (lat < -90 || lat > 90) {
    alert('Lat out of scope (-90, 90)');
    return;
  }
  if (lon < -180 || lon > 180) {
    alert('Lng out of scope (-180, 180)');
    return;
  }
...
function search(map, json, inputWrapper, value) {
  ...
  result.forEach((item) => {
    ...
    const row = $('<div>').addClass('item').html(`
    ...
    row.on('click', () => {
      map.setCenter(new google.maps.LatLng(item.loc.lat, item.loc.lon));
      popup(item);
