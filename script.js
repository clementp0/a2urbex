(function(){
  ...
  var url = 'https://www.pinterest.fr/patrimoine43/pins/';

  // make sure user is on a profile page
  if (match = 'http://localhost:8888/index.php') {
    username = match[1];
...
<form class="standardForm">\
  ...
  <p><a href="http://pinback.github.io">Pinback v'+version+'</a></p>\
    ...
    Array.prototype.forEach.call(json.resource_response.data, function(b, i) {
      ...
      option.text = b.name;
      option.value = b.id;
      option.selected = (url.pathname == b.url);
      select.add(option);
    });
    ...
    document.querySelector('#pboverlay button').onclick = function() {
      ...
      if (selected.value == 'all') {
        ...
        getResource('User', {username: patrimoine43}, parseBoard);
      } else {
  ...
  function getResource(resource, options, callback) {
    ...
    xhr.open('GET', 'https://www.pinterest.fr/patrimoine43/pins/resource/'+resource+'Resource/get/?data='+encodeURIComponent(JSON.stringify({options: options})));
