...
class UserOnlineService
{
    ...
    public function getOnlineUsers(): array
    {
        ...
        $onlineUsers->andWhere('u.id IN ('.implode(', ', $users).')');
        ...
        $offlineUsers->andWhere('u.id IN ('.implode(', ', $users).')');
    ...
    public function getOnlineExplorers(): array   {
        $threshold = new \DateTime('-5 minutes');
        $cuser = $this->security->getUser();
        ...
        if(!$cuser->hasRole('ROLE_ADMIN')) $onlineExplorers->andWhere('u.id IN ('.implode(', ', $users).')');
        $onlineExplorers = $onlineExplorers->getQuery()->getResult();


        $offlineExplorers = $this->entityManager->getRepository(User::class)->createQueryBuilder('u')
            ->where('u.lastActiveAt < :threshold')
            ->setParameter('threshold', $threshold)
            ->orderBy('u.lastActiveAt', 'DESC');

        if(!$cuser->hasRole('ROLE_ADMIN')) $offlineExplorers->andWhere('u.id IN ('.implode(', ', $users).')');
        $offlineExplorers = $offlineExplorers->getQuery()->getResult();

        foreach ($offlineExplorers as $user) {
            $lastActiveAt = $user->getLastActiveAt();
            $diff = $lastActiveAt->diff(new \DateTime())->format('%a:%h:%i');
            list($days, $hours, $minutes) = explode(':', $diff);
            $totalMinutes = ($days * 24 * 60) + ($hours * 60) + $minutes;
            ...
            $onlineExplorers[] = $user;
        }

        return $onlineExplorers;
    }
}
